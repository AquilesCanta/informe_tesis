...
El primero de los nuevos elementos es el servidor de \emph{streaming} que consiste en un \emph{host} con conexión a la red IP. Su responsabilidad es emitir el contenido audiovisual del servicio al que corresponde. Los paquetes deben ser dirigidos al grupo y puerto multicast indicados en la información provista a través del \emph{service elementaries relocation descriptor} de la norma ISDB-Tb extendida. 

Para asegurar esta correspondencia, se introduce una segunda entidad. El \emph{coordinador} decide qué servicios se emiten en la señal ISDB-Tb de forma tradicional y cuáles por la red IP. Define, además, qué grupo multicast y puerto se usan para la emisión. Con esta información se completa el \emph{services elementaries relocation descriptor} y se configura el servidor de \emph{streaming} multicast.


\subsection{Cabeceras de transmisión}

Una emisora ISDB-Tb modula una señal que puede ser directamente intepretada por un decodificador. Esto es, el flujo de transporte provee todas las capas de abstracción necesarias para la comunicación. De este modo, soluciona problemas como sincronización, multiplexación y corrección de errores.

En cambio, las redes IP se basan en el intercambio de paquetes entre nodos y respetan un modelo de capas. El término IP refiere a una de estas capas. Esto significa que el mecanismo con que estos paquetes se transmiten recae sobre los protocolos de capa inferior a la capa IP y los enlaces que unen a estos nodos. De forma inherente al modelo de capas, cada una agrega \emph{overhead} necesario para cumplir sus funciones.

Si un flujo de transporte es enviado a través de una red IP, como es el caso de los servicios reubicados, las diferentes capas incurren en un \emph{overhead} de comunicación, a veces redundante. Por ejemplo, los mecanismos de sincronización que provee el flujo de transporte son innecesarios en las redes IP. También, tanto el \emph{transport stream} MPEG como el protocolo UDP proveen \emph{checksum}\footnote{Si bien podría argumentarse que esto mejora la calidad de control de errores, la realidad es que viola la razón fundamental detrás del modelo de capas.}.

En términos de la \cref{fig:infra_extended}, el enlace entre el servidor de \emph{streaming} y la red IP resulta un eslabón clave en el esquema. Especialmente si se trata de un servidor que debe transmitir varios servicios por la red. La sobrecarga de esta vía implicaría que se vean afectados todos los servicios que emite en caso de alcanzar su límite de acho de banda. Por el contrario, es más más extraño encontrar sobrecargas sobre los enlaces de \emph{última milla}\footnote{Término utilizado en telecomunicaciones para referir al último extremo de la red que entrega conexión a los clientes.}. Se puede asumir que estos enlaces van a transportar un pequeño número de servicios simultáneamente, probablemente uno.
...
La emisión en las señales ISDB-Tb es \emph{broadcast}. En este esquema los consumidores son anónimos, lo cual implica que es imposible determinar si un receptor está consumiendo un servicio dado. En el caso de los servicios realocados, las redes IP identifican a los \emph{hosts} unívocamente a través de una dirección IP. Para saber si un receptor consume un servicio tranportado por IP, éste puede ser asociado a una dirección y determinar a qué grupo multicast se ha unido.
...
\begin{itemize}
	...
	\item El servicio destino se transmite por una red IP, y es señalizado por el flujo de transporte sintonizado actualmente. En el gráfico, servicios realocados, misma frecuencia.
	\item El servicio destino se transmite por la red IP, y es señalizado por un flujo de transporte ajeno al actual. En el gráfico, servicios realocados, distinta frecuencia.
\end{itemize}
