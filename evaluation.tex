...
Una transmisora de señal ISDB-Tb modula una señal que puede ser directamente intepretada por un decodificador. Esto es, el flujo de transporte provee todas las capas de abstracción necesarias para la comunicación. De este modo, soluciona problemas como sincronización, multiplexación y corrección de errores.

En cambio, las redes IP se basan en el intercambio de paquetes entre nodos. Pero la forma en que estos paquetes se transmiten recae sobre los protocolos de capa inferior a la capa IP y los enlaces que unen a estos nodos. Si un flujo de transporte es enviado a través de una red IP, como es el caso de los servicios realocados, las capas inferiores incurren en un \emph{overhead} de comunicación redundante, respecto de la señal ISDB-Tb plana. Por ejemplo, tanto el \emph{transport stream} MPEG y el protocolo UDP proveen \emph{checksum}. Los mecanismos de sincronización que provee el flujo de transporte son innecesarios en las redes IP, también.

En términos de la \cref{fig:infra_extended}, el enlace entre el servidor de \emph{streaming} y la red IP resulta un eslabón clave en el esquema. Especialmente si se trata de un servidor que debe transmitir varios servicios por la red. La sobrecarga de esta vía implicaría que se vean afectados todos los servicios que emite en caso de alcanzar su límite de acho de banda. Por el contrario, es más difícil encontrar sobrecargas sobre los enlaces de \emph{última milla}\footnote{Término utilizado en telecomunicaciones para referir al último extremo de la red que entrega conexión a los clientes.} porque no es de suponer que van a transportar un gran número de servicios simultáneamente.
...
La \cref{fig:max_serv_num} muestra cómo afectan las dos restricciones al número máximo de servicios señalizados por cada tabla. La barra roja indica cuántos servicios puede señalizar una tabla si ésta ocupa una sección completa y, por el otro, la barra azul indica cuántos servicios se pueden señalizar con un único paquete de flujo de transporte.
